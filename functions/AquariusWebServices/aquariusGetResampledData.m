function DataTable = aquariusGetResampledData(Host, SiteId, DataId, ...
                                              FromDate, ToDate, Interval, ...
                                              AuthToken, TimeZoneOutput, ...
                                              Username, Password)
%AQUARIUSGETRESAMPLEDDATA Get data from the Aquarius server
%   DataTable = AQUARIUSGETRESAMPLEDDATA(Host, SiteId, DataId, ...
%                                        FromDate, ToDate, Interval, ...
...
%   FromDate = The start time of the range to be returned specified as a 
%              datetime. 
%   ToDate   = The end time of the range to be returned specified as a 
%              datetime. 
%   Interval = Interval between resampled points specified as a duration.
...
%   AQUARIUSLISTDATASETS, AQUARIUSGETDATA
...
RequestString = sprintf('GetTimeSeriesDataResampled?token=%s&dataId=%s', ...
...
if exist('Interval','var') && ~isempty(Interval)
    RequestString = sprintf('%s&sampleIntervalInMinutes=%.0f',  ...
                            RequestString, minutes(Interval));
end
if exist('FromDate','var') && ~isempty(FromDate)
    RequestString = sprintf('%s&anchorTime=%s', RequestString, ...
                            datestr(FromDate, DateFormat));
end
