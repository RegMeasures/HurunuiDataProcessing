...
YTicsLst   = -600:600:1800; YLblLst   = {'Longshore transport, Q_s (m^3/day)'; ...
                                          '(positive northwards)'};
YTicsRunup = 1:1:4; YLblRunup = {'2% Runup height, R_{high}'; ...
                                 '(daily max, m-LVD)'};
YTicsLevel = -1:1:3; YLblLevel = {'Water level (daily range, m-LVD)'; ...
...
% runup plot (Poate et al 2016 formulation for runup)
...
% add shaded bands for overtopping and overwashing
RunupH{end+1} = patch(ax(2),datenum([XRange,fliplr(XRange)]), ...
                      ...
                      [0.85,0.9,0.85], 'EdgeColor', 'none', 'FaceAlpha', 0.6);
uistack(ChanH{end},'bottom')
% RunupH{end+1} = plot(ax(2), repmat(datenum(XRange(1)+days(90)), [2,1]), ...
%                      Config.CrestHeight * DataScale + DataOffset, 'Color', [0.2,0.3,0.2]);
...
LegChan = legend(ax(2),[ChanH{1},ChanH{2}], ...
                 {'Lagoon end of outlet channel', ...
                  'Sea end of outlet channel'});               
