...
rangeFill = 10 * LevelScale;
range = max(DailyLagoonTS.MaxWL(DailyLagoonTS.Date>=XRange(1) & ...
                                DailyLagoonTS.Date<=XRange(2))) -  ...
        min(DailyLagoonTS.MinWL(DailyLagoonTS.Date>=XRange(1) & ...
                                DailyLagoonTS.Date<=XRange(2)));
scale = rangeFill/range;
mdp = (min(DailyLagoonTS.MinWL(DailyLagoonTS.Date>=XRange(1) & ...
                               DailyLagoonTS.Date<=XRange(2))) + ...
      range/2) * scale;
LightBlue = [47/256, 141/256, 245/256];
patch(ax(2), ...
      datenum([DailyLagoonTS.Date; flipud(DailyLagoonTS.Date)]), ...
      [DailyLagoonTS.MinWL; flipud(DailyLagoonTS.MaxWL)] * ...
          scale - 55 - mdp + LevelOffset, ...
      [47/256, 141/256, 245/256], 'EdgeColor', 'none')

LevelH = subplot_ts(ax, 6, YTicsLevel, YLblLevel, LevelScale, LevelOffset, ...
                    's1', datenum(DailyLagoonTS.Date), DailyLagoonTS.MaxWL, '-', 'Color', LightBlue, ...
                    's2', datenum(DailyLagoonTS.Date), DailyLagoonTS.MinWL, '-', 'Color', LightBlue);%, ...
                    %'s3', datenum(DailyLagoonTS.Date), DailyLagoonTS.MaxSeaLevel, '-k');

% Legends
LegChan = legend(ax(2),[ChanH{1},ChanH{2}], ...
                 {'Upstream end of outlet channel', ...
                  'Downstream end of outlet channel'});               
set(LegChan, 'Position', [0.08, 0.88, 0.2, 0.0406], ...
